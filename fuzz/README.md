...
| `fuzz_backend_buffers` | Windows `RawSMBIOSData`, raw-buffer loading used by macOS, and the legacy Linux entry-point scan |
...
The platform-neutral parts of the Windows, macOS, and legacy Linux backends are
fuzzed by `fuzz_backend_buffers`. Calls into `GetSystemFirmwareTable`, IOKit,
sysfs, `/dev/mem`, `mmap`, and permission handling remain platform integration
