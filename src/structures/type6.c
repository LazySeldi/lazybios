...
lazybiosType6Array_t* lazybiosGetType6(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_MEMORY_MODULE);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_MEMORY_MODULE].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_MEMORY_MODULE].first;
	}
	size_t index = 0;
