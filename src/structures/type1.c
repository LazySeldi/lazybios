...
lazybiosType1Array_t* lazybiosGetType1(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_SYSTEM);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_SYSTEM].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_SYSTEM].first;
	}
	size_t index = 0;
