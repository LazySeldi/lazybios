...
lazybiosType10Array_t* lazybiosGetType10(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_ONBOARD_DEVICES);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_ONBOARD_DEVICES].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_ONBOARD_DEVICES].first;
	}
	size_t index = 0;
