...
lazybiosType3Array_t* lazybiosGetType3(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_CHASSIS);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_CHASSIS].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_CHASSIS].first;
	}
	size_t index = 0;
