...
lazybiosType8Array_t* lazybiosGetType8(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_PORT_CONNECTOR);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_PORT_CONNECTOR].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_PORT_CONNECTOR].first;
	}
	size_t index = 0;
