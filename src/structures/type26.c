...
lazybiosType26Array_t* lazybiosGetType26(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_VOLTAGE_PROBE);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_VOLTAGE_PROBE].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_VOLTAGE_PROBE].first;
	}
	size_t index = 0;
