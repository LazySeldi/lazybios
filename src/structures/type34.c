...
lazybiosType34Array_t* lazybiosGetType34(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_MANAGEMENT_DEVICE);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_MANAGEMENT_DEVICE].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_MANAGEMENT_DEVICE].first;
	}
	size_t index = 0;
