...
lazybiosType45Array_t* lazybiosGetType45(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_FIRMWARE_INVENTORY_INFORMATION);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_FIRMWARE_INVENTORY_INFORMATION].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_FIRMWARE_INVENTORY_INFORMATION].first;
	}
	size_t index = 0;
