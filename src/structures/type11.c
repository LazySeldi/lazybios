...
lazybiosType11Array_t* lazybiosGetType11(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_OEM_STRINGS);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_OEM_STRINGS].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_OEM_STRINGS].first;
	}
	size_t index = 0;
