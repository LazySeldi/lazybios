...
lazybiosType24Array_t* lazybiosGetType24(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_HARDWARE_SECURITY);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_HARDWARE_SECURITY].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_HARDWARE_SECURITY].first;
	}
	size_t index = 0;
