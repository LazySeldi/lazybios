...
lazybiosType19Array_t* lazybiosGetType19(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_MEMORY_ARRAY_MAPPED_ADDRESS);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_MEMORY_ARRAY_MAPPED_ADDRESS].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_MEMORY_ARRAY_MAPPED_ADDRESS].first;
	}
	size_t index = 0;
