...
lazybiosType25Array_t* lazybiosGetType25(const lazybiosDMI_t* DMIData) {
	...
	if (DMIData->index_valid != 1) {
	    count = lazybiosCountStructsByType(DMIData, SMBIOS_TYPE_SYSTEM_POWER_CONTROLS);
	    p = DMIData->dmi_data;
	} else {
	    count = DMIData->index[SMBIOS_TYPE_SYSTEM_POWER_CONTROLS].count;
	    p = DMIData->dmi_data + DMIData->index[SMBIOS_TYPE_SYSTEM_POWER_CONTROLS].first;
	}
	size_t index = 0;
